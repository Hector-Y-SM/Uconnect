...
const CommentModal = ({
  ...
  const deleteComment = async (comment_uuid: string) => {
    const { error } = await supabase.from("comments").delete().eq("comment_uuid", comment_uuid);
  ...
  const goToUserProfile = (userUuid: string | undefined) => {
    if (!userUuid) return;
    onClose(); 
    router.push(`../screens/userProfileScreen?userUuid=${userUuid}`);
  };

  const renderCommentItem = ({ item }: { item: CommentWithUserInfo }) => (
    <View className="flex-row p-4 border-b border-gray-100">
      <TouchableOpacity onPress={() => goToUserProfile(item.user_uuid)} activeOpacity={0.7}>
        <Image
          source={{
      ...
      {item.user_uuid === userUUID ? (
        <View className="flex-row">
          <TouchableOpacity onPress={() => editComment(item.comment_uuid)} className="mr-3 p-1">
            <Text className="text-blue-500">Editar</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => deleteComment(item.comment_uuid)} className="p-1">
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <View className="px-4 py-3 border-b border-gray-100">
              <Text className="text-sm text-gray-600">Publicación de @{postAuthor}</Text>
            </View>
          }
          ListEmptyComponent={
            <View className="flex-1 justify-center items-center py-10">
